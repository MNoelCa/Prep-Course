...
  // Si no se pasan argumentos devuelve 0. Si se pasa un argumento, simplemente devuélvelo
  ...
  if (arguments.length === 0) {
    return 0;
    // si hay cero argumentos, retorna cero
  } else if (arguments.length === 1) {
    return arguments[0]
    //si hay un solo elemento, retorna el argumento de la primer posición[0]
  } else {
...
function todosIguales(arreglo) {
  ...
  //Escribe tu código aquí  
  var arreglo0 = arreglo[0]
  for (i=1; i<arreglo.length; i++) {
    if (arreglo0 !== arreglo[i]) {
      return false
    }
  }
  return true
  ...
  for (i=0; i<10; i++) {
    sum = sum+2;
      if (sum === i) {
        ...
        //debido a que 'return' corta la iteración, no comprendo donde deberíamos usar 'break'
      }
    NuevoArray.push(sum); 
  }
  return NuevoArray;
